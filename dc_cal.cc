...
void analysis(Int_t number = 1)
{
  ...
  bdc->Branch("bdc1idNum",&bdc1idNum,"bdc1idNum/I");
  bdc->Branch("bdc1NumHitLayer",&bdc1NumHitLayer,"bdc1NumHitLayer/I");
  bdc->Branch("bdc1NumDCTrack",&bdc1NumDCTrack,"bdc1NumDCTrack/I");

  //Branch for std::vector//
  bdc->Branch("bdc1layer",&bdc1layer);
  bdc->Branch("bdc1wireID",&bdc1wireID);
  bdc->Branch("bdc1planeID",&bdc1planeID);
  bdc->Branch("bdc1tdc",&bdc1tdc);
  bdc->Branch("bdc1dl",&bdc1dl);

  bdc->Branch("bdc1isX",&bdc1isX);
  bdc->Branch("bdc1wireX",&bdc1wireX);
  bdc->Branch("bdc1wireY",&bdc1wireY);
  ...
  while(eventStore->GetNextEvent()&&(!nMaxEvent || nEvent<nMaxEvent))
      ...
      for(Int_t i=0;i<NumDCHit;i++)
	{
	  ...
	  bdc1wireID.push_back( hit->GetWireID() );
	  bdc1planeID.push_back( hit->GetPlaneID() );
	  bdc1tdc.push_back( hit->GetTDC() );
	  bdc1wireX.push_back( hit->GetWirePosition() );
	  bdc1wireY.push_back( hit->GetWirePosition() );
      ...
      for(Int_t j=0;j<NumDCHit;j++)
	{
	  for(Int_t track=0;track<NumDCTrack;track++)
		...
		break;
	      ...
	      bdc1NumHitLayer =  trk1->GetNumHitLayer();
...
void dc_cal()
{
  ...
  c1->Divide(2,1);
  c1->cd(1);
  tree->Draw("abs(bdc1calX-bdc1wireX):bdc1tdc>>h1(120,1640,1760,200,0,8)","bdc1layer==0","colz");
  c1->cd(2);
  tree->Draw("abs(bdc1calY-bdc1wireY):bdc1tdc>>h2(120,1640,1760,200,0,8)","bdc1layer==2","colz");

  TCanvas *c2 = new TCanvas("c2","c2",1200,600);
  c2->Divide(2,1);
  c2->cd(1);
  tree->Draw("abs(bdc1hitX-bdc1wireX):bdc1tdc>>h3(120,1640,1760,200,0,8)","bdc1layer==0","colz");
  c2->cd(2);
  tree->Draw("abs(bdc1hitY-bdc1wireY):bdc1tdc>>h4(120,1640,1760,200,0,8)","bdc1layer==2","colz");
