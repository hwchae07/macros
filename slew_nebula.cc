void slew_nebula()
{
  ...
  for(Int_t id=1;id<=120;id++)
      ...
      if(ts->GetNPeaks()!=2)
	{
      ...
      c2->cd(1);
      chain->Draw(Form("nebulaTOF:TMath::Power(nebulaQPed,-0.25)>>h4ID%d(100,0.2,0.4,400,-280,-230)",id),Form("nebulaID==%d&&abs(nebulaTA-%lf-2)<3",id,center),"colz");
      ...
      c2->cd(2);
      chain->Draw(Form("nebulaTOF:TMath::Power(nebulaQPed,-0.25)>>h5ID%d(50,0.2,0.4,400,-280,-230)",id),Form("nebulaID==%d&&abs(nebulaTA-%lf-2)<3",id,center),"prof");
      h5 = (TProfile*)gDirectory->Get(Form("h5ID%d",id));
      h5->GetYaxis()->SetRangeUser(d4,u4);
      TF1 *fit1 =new TF1("fit1","pol2",0.26,0.36);
      fit1->SetParameter(0,center);
      h5->Fit("fit1","rq","",0.26,0.36);
      ...
      for(Int_t i=0;i<7;i++)
	{
      ...
      c2->cd(3);
      chain->Draw(Form("nebulaTOF-%lf*TMath::Power(nebulaQPed,-0.25)-%lf*TMath::Power(nebulaQPed,-0.5):TMath::Power(nebulaQPed,-0.25)>>h6ID%d(100,0.2,0.4,400,-280,-230)",slewPar1,slewPar2,id),Form("nebulaID==%d&&abs(nebulaTA-%lf-2)<3",id,center),"colz");
      ...
      c2->cd(4);
      chain->Draw(Form("nebulaTOF:TMath::Power(nebulaQPed,-0.25)>>h7ID%d(100,0.2,0.4,400,-280,-230)",id),Form("cut1ID%d&&nebulaID==%d&&abs(nebulaTA-%lf-2)<3",id,id,center),"goff");
      ...
      c2->cd(5);
      chain->Draw(Form("nebulaTOF:TMath::Power(nebulaQPed,-0.25)>>h8ID%d(50,0.2,0.4,400,-280,-230)",id),Form("cut1ID%d&&nebulaID==%d&&abs(nebulaTA-%lf-2)<3",id,id,center),"prof");
      h8 = (TProfile*)gDirectory->Get(Form("h8ID%d",id));
      h8->GetYaxis()->SetRangeUser(d4,u4);
      TF1 *fit2 = new TF1("fit2","pol2",0.26,0.36);
      fit2->SetParameters(fit1->GetParameters());
      h8->Fit("fit2","rq","",0.26,0.36);
      ...
      for(Int_t i=0;i<7;i++)
	{
      ...
      Int_t nentry = chain->GetEntries(Form("cut2ID%d&&nebulaID==%d&&abs(nebulaTA-%lf-2)<3",id,id,center));
      if(nentry>100)
      ...
      else
	{
      ...
      c2->cd(4);
      chain->Draw(Form("nebulaTOF:TMath::Power(nebulaQPed,-0.25)>>h10ID%d(100,0.2,0.4,400,-280,-230)",id),Form("cut2ID%d&&nebulaID==%d&&abs(nebulaTA-%lf-2)<3",id,id,center),"colz");
      ...
      c2->cd(5);
      chain->Draw(Form("nebulaTOF:TMath::Power(nebulaQPed,-0.25)>>h11ID%d(50,0.2,0.4,400,-280,-230)",id),Form("cut2ID%d&&nebulaID==%d&&abs(nebulaTA-%lf-2)<3",id,id,center),"prof");
      ...
      fout<<id<<" "<<slewPar1<<" "<<slewPar2<<" "<<h4->GetEntries()<<endl;
      ...
      c2->cd(6);
      chain->Draw(Form("nebulaTOF-%lf*TMath::Power(nebulaQPed,-0.25)-%lf*TMath::Power(nebulaQPed,-0.5):TMath::Power(nebulaQPed,-0.25)>>h9ID%d(100,0.2,0.4,400,-280,-230)",slewPar1,slewPar2,id),Form("nebulaID==%d&&abs(nebulaTA-%lf-2)<3",id,center),"colz");
...
void slew_additional(int i=0)
{
  ...
  for(i=0;i<37;i++)
	...
	return;
      ...
      if(par2>0)
	fout<<id<<" "<<par1<<" "<<par2<<" "<<h1->GetEntries()<<endl;  
  ...
  */
  ...
  while(1)
      ...
      if(fin1.fail())
	{
      ...
      fin1>>id[i]>>par1[i]>>par2[i]>>eventNum[i];
      ID=id[i];
      p1=par1[i];
      p2=par2[i];
      entry=eventNum[i];
  ...
  while(1)
      ...
      if(fin2.fail())
	{
      ...
      double dummyID, dummyP1, dummyP2, dummyEN;
      fin2>>dummyID>>dummyP1>>dummyP2>>dummyEN;
      for(int j=0;j<200;j++)
	{
	  if(dummyID == id[j])
	    {
	...
	continue;
      ...
      par2[i] = dummyP2;
      eventNum[i] = dummyEN;
      dummyID=0;dummyP1=0;dummyP2=0;dummyEN=0;
  ...
  for(i=0;i<120;i++)
	...
	{
	  NN++;
	  avg_par1 += par1_last[i];
	  avg_par2 += par2_last[i];
  ...
  for(i=0;i<120;i++)
    {
      ...
      cout<<i+1<<" "<<id_last[i]<<" "<<par1_last[i]<<" "<<par2_last[i]<<" "<<eventNum_last[i]<<endl;
      fout<<id_last[i]<<" "<<par1_last[i]<<" "<<par2_last[i]<<" "<<eventNum_last[i]<<endl;
    }
