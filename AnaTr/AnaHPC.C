...
void AnaHPC::Analysis(){
  ...
  */
  ...
  for (Int_t i = 0 ; i < fCalibNEBULA->GetNumNEBULAPla() ; i++)
	...
	if(TUCal<0)
	  {
	    ...
	    nebulaDTS[nebulaNum] = TMath::QuietNaN();
	  }
	if(TDCal<0)
	  {
	...
	nebulaY2[nebulaNum]
	  = par_ycal0[nebulaID[nebulaNum]-1]
	  + par_ycal1[nebulaID[nebulaNum]-1] * (-1*nebulaDTS[nebulaNum]);

	if(nebulaID[nebulaNum]>120)
	  nebulaY2[nebulaNum] = nebulaY[nebulaNum];

	if(TUCal<0 || TDCal<0)
	  nebulaY2[nebulaNum] = -9999;
