...
void Analysis(Int_t runNum) {
  ...
  Double_t fragCharge = 8;
  Double_t fragMass = 20498.06444; // [MeV/c^2]
  ...
  TCutG* fragA = Load2DGate("cut/f22o.cut");

  TFile* saveFile = new TFile(Form("./root/run%04d.root.Erel",runNum),"RECREATE");
  ...
  for (Int_t i = 0 ; i < tree->GetEntries(); i++){
    ...
    ////////////////////////////////////////////////////////////////
    // MINOS
    Double_t tgt0X = bdc2X+(LBDC2_MINF+LBDC2_MINF+LMINF_MIN0-LBDC/2)/LBDC1_BDC2*(bdc2X-bdc1X);
    Double_t tgt0Y = bdc2Y+(LBDC2_MINF+LBDC2_MINF+LMINF_MIN0-LBDC/2)/LBDC1_BDC2*(bdc2Y-bdc1Y);
    Double_t tgt0A = LBDC1_BDC2*(bdc2X-bdc1X);
    Double_t tgt0B = LBDC1_BDC2*(bdc2Y-bdc1Y);
